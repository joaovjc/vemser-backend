...
public interface PessoaRepository extends JpaRepository<PessoaEntity, Integer> {
	...
	@Query(nativeQuery = true, value = "SELECT * FROM PESSOA p WHERE extract(month from data_nascimento) = extract(month from sysdate) AND extract(day from data_nascimento) = extract(day from sysdate)")
	List<PessoaEntity> listPorAniverHoje();
}
